    ...
    try:
        ...
        if iter_cnt >= MAX_HYPOTHESES:
            return {
                ...
                "messages": [{"role": "eda", "content": "Cap reached. Moving to visualization."}]
            }
...
Return ONLY valid JSON. No markdown."""
        ...
        if test_type == "correlation":
            test_result = correlation_matrix(dataset)
        elif test_type == "group_comparison" and len(columns) == 2:
            test_result = group_comparison(dataset, columns[0], columns[1])
        elif test_type == "value_counts" and columns:
            test_result = value_counts(dataset, columns[0])
        elif test_type == "distribution" and columns:
            test_result = distribution_stats(dataset, columns[0])
...
Return ONLY the JSON object. No markdown. No explanation. No extra text."""
        ...
        new_iter = iter_cnt + 1
        next_phase = "visualization" if new_iter >= MAX_HYPOTHESES else "eda"
        ...
        return {
            "errors": [{"agent": "eda", "error": str(e), "phase": "eda"}],
            "current_phase": "visualization"
        }